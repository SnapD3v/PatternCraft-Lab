...
class TheoryManager(ContentManager):
	...
	def add_theory_text(self, name: str, content: str, description: str, image_url: str, block_id: int) -> Tuple[bool, Optional[str]]:
		...
		texts_block = session.query(db.TextsBlock).filter(db.TextsBlock.id == block_id).first()
	...
	def get_texts_blocks(self) -> List[db.TextsBlock]:
		session = self.session_factory()
		result: List[db.TextsBlock] = session.query(db.TextsBlock).options(db.joinedload(db.TextsBlock.texts)).all()
		session.close()
		return result

	def get_theory(self, theory_text_id: int) -> db.TheoryText:
		session = self.session_factory()
		result: db.TheoryText = session.query(db.TheoryText.id == theory_text_id).first()
		session.close()
		return result
...
class SolutionHistoryElementDTO:
	def __init__(self, solution_history_element: db.SolutionHistoryElement) -> None:
		...
		if self.type == 'text':
			...
			self.content = markdown.markdown(raw_content, extensions=['fenced_code', 'nl2br'])
		else:
...
class ProblemDTO:
	def __init__(self, problem: db.Problem) -> None:
		...
		self.solution_history_elements = [SolutionHistoryElementDTO(i) for i in problem.solution_history_elements]
...
class ProblemsManager(ContentManager):
	...
	def get_problems_set(self, tags: List[str]) -> int:
		...
		problems_set = session.query(db.ProblemsSet).filter(db.ProblemsSet.tags_json == tags_json).first()
		if problems_set:
	...
	def create_problem(self, tags: List[str]) -> int:
		...
		problems_set = session.query(db.ProblemsSet).filter(db.ProblemsSet.id == problems_set_id).first()
...
class WebRoutes(IRouteProvider):
	...
	def register(self, app: Flask) -> None:
		...
		app.add_url_rule("/create/problem", view_func=self.create_problem, methods=["GET", "POST"])
		app.add_url_rule("/theory/<int:id>", view_func=self.theory, methods=["GET"])
		app.add_url_rule("/problem/<int:id>", view_func=self.problem, methods=["GET", "POST"])
...
if __name__ == '__main__':
	api_key = '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'
	...
	writer_user_prompt = 'Придумай задачу, решение которой потребует применения: {tags}. Напиши на первой строке название задачи, а после нее подробное ТЗ.'
	task_writer: Agent = TaskWriter(text_generator, writer_system_prompt, writer_user_prompt)

	reviewer_prompt = 'Проверь, правильно ли решена задача. Проверь, соблюдены ли принципы и паттерны: {tags}.'
	reviewer: Agent = Reviewer(text_generator, reviewer_prompt)
	...
	web_app_config: Dict[str, Any] = {
		'host': '0.0.0.0',
		'port': 8000,
		'debug': True
	}

	routes = WebRoutes(theory_manager, problems_manager, tags)
	app = WebAppFactory.create_app(routes)
	runner = AppRunner(app, web_app_config)
	runner.run()