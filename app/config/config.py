...
class PromptConfig(BaseModel):
    ...
    writer_user_prompt: str = (
        "Придумай задачу, решение которой потребует применения: {tags}. Напиши на первой строке название задачи, а после нее подробное ТЗ."
    )
    reviewer_prompt: str = (
        "Проверь, правильно ли решена задача. Проверь, соблюдены ли принципы и паттерны: {tags}."
    )
...
class TagConfig(BaseModel):
    ...
    def tags_list(self) -> list[str]:
        return [*self.creational, *self.structural, *self.behavioral, *self.solid]
